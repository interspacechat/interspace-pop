...
const addOrDeleteVideo = (videoId, win, type, action) => {
  ...
  document.querySelectorAll('#settings span.nr-of-mappertjes').forEach(span => {
...
const getVideoDocUrl = (videoId, enableMappertje) => {
  return `about:blank#/extId=${extId}/id=${videoId}/mappertje=${enableMappertje ===
    true}`
}
...
const makeMultiviewWindow = () => {
  ...
  if (!jitsipop.multiviewWindow || jitsipop.multiviewWindow.closed) {
    jitsipop.multiviewWindow = window.open(
      getMultiviewDocUrl(),
      'multiview',
      `status=no,menubar=no,width=${popupWidth},height=${popupHeight},left=${screen.availLeft +
        xOffset},top=${screen.availTop + yOffset}`
    )
...
const updateContextbar = () => {
  ...
  contextbar.querySelector('h4').innerText = formatDisplayName(item.displayName)
...
const setupContextbar = () => {
  ...
  contextbar.querySelector('.mappertje-button').addEventListener('click', e => {
    e.preventDefault()
    popOutVideo(selectedVideoId, true)
  })

  contextbar.querySelector('.multiview-button').addEventListener('click', e => {
    e.preventDefault()
